    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans antialiased pb-16">
      ...
      <div className="bg-slate-900/90 border-b border-slate-800 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 py-4 flex items-center justify-between">
          <button
            onClick={onBack}
            className="group flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-white bg-slate-800/80 hover:bg-slate-800 px-3.5 py-2 rounded-xl border border-slate-700/60 transition-all cursor-pointer"
          >
      ...
      {/* Main Stock Detail Page Container */}
      <main className="max-w-7xl mx-auto px-4 sm:px-8 py-8 space-y-8">
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-bold text-white font-mono tracking-tight">{stock.symbol}</h1>
              <span
            ...
            <p className="text-sm text-slate-400 mt-1">{stock.company_name || stock.symbol}</p>
          </div>

          <div className="text-xs text-slate-400 font-mono bg-slate-800/40 border border-slate-700/50 rounded-xl px-4 py-2.5 flex items-center gap-4">
            <div>
              <span className="text-slate-500 block text-[10px]">SCANNER DATE</span>
              <span className="text-slate-200 font-semibold">{stock.signal_date || 'Latest'}</span>
            </div>
            <div className="h-6 w-px bg-slate-700/60" />
            <div>
              <span className="text-slate-500 block text-[10px]">STRATEGY</span>
              <span className="text-slate-200 font-semibold">{stock.strategy_name} v{stock.strategy_version}</span>
            </div>
          </div>
        </div>
        ...
        <section className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-4">
              <span className="text-xs text-slate-400 block mb-1">Entry Price</span>
            ...
            <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-4">
              <span className="text-xs text-slate-400 block mb-1">Target Price</span>
            ...
            <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-4">
              <span className="text-xs text-slate-400 block mb-1">Risk : Reward</span>
        ...
        <section className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6">
          ...
          <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-5 text-xs text-slate-300 leading-relaxed font-mono">
          ...
          <section className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              {Object.entries(stock.metadata).map(([key, val]) => (
                <div key={key} className="bg-slate-800/40 border border-slate-700/40 rounded-xl p-3 flex justify-between items-center">
                  <span className="text-slate-400 font-mono">{key}</span>
                  <span className="font-bold text-slate-200 font-mono">{String(val)}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        {backtestError && (
          <div className="bg-rose-950/30 border border-rose-500/40 rounded-2xl p-5 text-xs text-rose-300 font-mono">
        ...
        {backtestResult && (
          <section className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              ...
              <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-3.5">
                <span className="text-slate-400 block text-[11px]">Winning / Losing</span>
                <span className="text-xl font-bold font-mono text-emerald-400">
                  ...
                  <table className="w-full text-left text-xs font-mono">
                    <thead className="bg-slate-800/80 text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-700/60">
                      <tr>
                        ...
                        <th className="py-3 px-3 text-right">R-Mult</th>
                      </tr>
                    </thead>
                      ...
                      {backtestResult.trades.map((t, idx) => {
                        ...
                        return (
                          <tr key={t.trade_id || idx} className="hover:bg-slate-800/40 transition-colors">
                            <td className="py-2.5 px-3 text-slate-400">{t.signal_date || t.entry_date}</td>
                            <td className="py-2.5 px-3 text-slate-300">{t.entry_date}</td>
                            <td className="py-2.5 px-3 text-emerald-400 font-bold">₹{t.entry_price.toFixed(2)}</td>
                            <td className="py-2.5 px-3 text-rose-400">{t.stop_loss ? `₹${t.stop_loss.toFixed(2)}` : '-'}</td>
                            <td className="py-2.5 px-3 text-teal-300">{t.target_price ? `₹${t.target_price.toFixed(2)}` : '-'}</td>
                            <td className="py-2.5 px-3 text-slate-300">{t.exit_date}</td>
                            <td className="py-2.5 px-3 text-slate-200 font-bold">₹{t.exit_price.toFixed(2)}</td>
                            ...
                            <td className={`py-2.5 px-3 text-right font-bold ${t.r_multiple && t.r_multiple > 0 ? 'text-emerald-400' : t.r_multiple && t.r_multiple < 0 ? 'text-rose-400' : 'text-slate-400'}`}>
        ...
        <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4 text-[11px] text-slate-400">
